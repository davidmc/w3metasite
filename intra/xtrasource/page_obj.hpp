/*
  ...
  Copyright (late) 2004 - 2007 by D.K. McCombs.
  ======================================

  -- New 2007
  The control template changes:
  
  menu item needs a name attribute that matches the name of a menu in the database.
...
public:
  ...
  { 
    ...
    if( changes.find("url") != changes.end() )
    {
      ...
      ocString sql = "select url from pages where id = ";
      sql.append(id);
      if(rs.open(sql))
      {      
        // update links with that url to have the new url
        sql = "update links set url = '" + url + "' where site_id = ";
        sql.append(site_id);
        sql += " and url = '";
  ...
  {
    ocString sql = "delete from page_menus where page_id = ";
    sql.append(id);
    cmd.execute(sql);
  }
  void deletePageParagraphs( void )
  {
    ocString sql = "delete from paragraphs where page_id = ";
    sql.append(id);
    cmd.execute(sql);
  }
      ...
      if( tmplt.get_data() )
      {
        ...
        if( cTemp.load(fullPath.c_str()) )
        {
          ...
          while( it !=  parser.states.nodes.end() )
          {
            ...
            string collapsible = node.attr["collapsible"];

            class menu mnu;
            ocString clause = "name = '";
            clause += name_tag;
            clause += "' and site_id = ";
            clause.append(site_id);
            ...
            if( mnu.id )
            {
              ...
              if(  collapsible == "true" ) pg_mnu.collapsible = true;
              pg_mnu.db_insert();
            }
