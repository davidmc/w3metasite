...
public:
  ...
  { 
    ...
    else
    {
      ...
      if( place_order == 0 ) // the default
      {
        ...
        ocString msql = "select max(place_order) from menu_items "
                        "where menu_id = ";
        msql.append( menu_id );
        if(rs.open(msql))
        {
  ...
  bool usupplemental( changeMap & changes )
  {
    ...
    {
      ocString mvSql = "update menu_items set place_order = place_order+1 where place_order >= ";
  ...
  bool move( long long mid, menu_item_direction dir, long absPos=0 )
  {
    ...
    if(bret)
    {
      ...
      switch(dir)
      {
        ...
        mvSql = "select id, place_order from menu_items where place_order < ";
        mvSql.append(place_order);
        mvSql += " and menu_id = ";
        mvSql.append(menu_id);
