/*
  ...
  NEEDS: Templates/imageListing.htmp
*/
...
public:
  ...
  ,filter(LanczosFilter),quality(75),m_relPath("")
  {
    string siteLimitation = siteFocus(login);
    string sql = "select path, domain, url from metasite.sites";
  ...
  void  imageList(cgiScript & script)
  {
    ...
    writelog("Loading Templates/imageListing.htmp");
    htmlDoc.load("Templates/imageListing.htmp");
    ...
    script << ocString(htmlDoc.getParagraph("top"))
           ...
           << endl;
    // DBG script << "Path: " <<  m_thumbDir << "<BR>" << endl;      
    ...
    script << headitem.replace("__hcell_label","Pick");       
    script << headitem.replace("__hcell_label","Delete");

    ocString iLnk = "<img src='_img_'>";

    ocString fLnk = "<a href='javascript:pickImage(\"_img_\")'>Full Size</a></div><br>"
                    "<a href='javascript:pickImage(\"_img_\")'>Thumbnail</a></div>"   ;
