...
/*
  Artisan UI - Glues the business object to the form object through 
...
public: 
  ...
  bool list_display( void )
  {
    ...
    editLink = editLink.replace("$prog$","artisan_ui.cgi");
    hotCol=1;    
    emitFilter( "artisan_ui.cgi",  // program name
                "&nbsp;&nbsp;&nbsp;&nbsp;<B>FILTER (by Last Name)</B>" );  // Label
    string heading = " Id | Name ";
    emitHeadings(heading);

    getFilteredData( "Id, concat(First_Name, ' ', Last_Name) as Name", 
                     "Person p inner join Artisan a"
                     " on p.Id = a.Person_Id ", 
                     "Last_Name like '$filter$%'", 
                     "Last_Name, First_Name" );
    emitData();  
    emitNavigation("artisan_ui.cgi");     
    emitEnd();
    return breturn;
  }      
}; 
...
public:
  ...
  {
    bool breturn = true;
    script  << makeTop( "artisan_ui.cgi", "Artisan" )
            ...
            << makeComboBox("Artisan Group","Artisan_Group_Id"
                            ...
                            ,"Select Group (if any)")    
            << makeTextArea("Biography","Biography",Biography,"5","80")  
