/*
  openMetaLogin.h (non private, uses metasite schema)
...
public:
  // now part of the interface, not used here yet
  string token;
  ...
  virtual bool checkUser( const char * user, const char * password )
  {
    ...
    if( user && password && strlen(user) && strlen(password) )
    {
      ...
      sql = "Select u.id,u.group_id,u.first,u.last," // 0,1,2,3
            ...
            " from metasite.users u inner join metasite.groups g on g.id=u.group_id ";
      sql += " where u.login = '";
      sql += user;
      sql += "' and u.password = '";
      sql += password;
  ...
  virtual bool testLoginStatus( void )
  {
    ...
    if( rId.length() > 0 )
    {
      ...
      sql = "Select u.id,u.group_id,u.first,u.last,"
            ...
            " from metasite.users u inner join metasite.groups g on g.id=u.group_id where u.id = ";
      sql += rId;
      if( rs.open(sql) )
      {
        // set the data vars
  ...
  bool fetchServices( string criteria = "" )
  {
    ...
    sql = "select distinct s.id, s.name, s.menu_name, s.uri, "// 0,1,2,3
          ...
          "inner join metasite.group_services gs on gs.service_id = s.id "
          "inner join metasite.role_services rs on rs.service_id = gs.service_id "
          "inner join metasite.roles r on r.id = rs.role_id "
          "inner join metasite.user_roles ur on ur.role_id = r.id "
          "where ur.user_id = ";
    sql += id;
