...
static void LOG_TIME(const char *msg)
{
	...
	FILE *fp = fopen("/tmp/tclink.log", "a");
	if (!fp)
		return;

	gettimeofday(&tv, 0);
	strftime(buf2, sizeof(buf2)-1, "%F %T", localtime(&tv.tv_sec));
	sprintf(buf, "[%s %03dms] %s\n", buf2, (int)tv.tv_usec / 1000, msg);
	fputs(buf, fp);
	fclose(fp);
}
...
static int BeginConnection(TCLinkCon *c, int host_ip)
{
	...
	LOG_TIME("BeginConnection() start");
	...
	LOG_TIME("BeginConnection() finish");
	return sd;
}
...
static int FinishConnection(TCLinkCon *c, int sd)
{
	...
	LOG_TIME("FinishConnection() begin");
	...
	if (is_error) {
		SSL_free(c->ssl);
		LOG_TIME("FinishConnection() finish (error)");
		return 0;
	}
	...
	if (!server_cert || (X509_cmp(server_cert, c->tc_cert) != 0)) {
		LOG_TIME("FinishConnection() finish (error, bad cert)");
		SSL_free(c->ssl);
		return 0;
	}
	...
	LOG_TIME("FinishConnection() finish (success)");
	return 1;
}
...
static int CheckConnection(TCLinkCon *c, int *sd, int num_sd)
{
	...
	LOG_TIME("CheckConnection() start");
		...
		if (sd[i] >= 0)
		{
			...
			else if (FD_ISSET(sd[i], &wr_set))
			{
				...
				if (FinishConnection(c, sd[i])) {
					...
					c->sd = sd[i];
					LOG_TIME("CheckConnection() finish (success)");
					return i;
				}
				else {
	...
	LOG_TIME("CheckConnection() finish (failure)");
...
static int Connect(TCLinkCon *c, int host_hash)
{
	...
	LOG_TIME("starting hostname lookup");

	/* determine IP addresses of gateway */
	if (!c->ip) 
	{
		LOG_TIME("gethostbyname begin");
		he = gethostbyname(tclink_host);
		LOG_TIME("gethostbyname end");
		if (he)
			c->dns = 1;
		else {
...
void TCLinkSend(TCLinkHandle handle)
{
	...
	if (!Connect(c, host_hash))
	{
		...
		LOG_TIME("------------ Transaction End (error) -----------\n");
		return;
	}

	LOG_TIME("connection established, sending data");
	...
	if (Send(c, buf))
	{
		LOG_TIME("send complete, waiting for response");
	...
	LOG_TIME("transaction send/result complete, closing connection");
